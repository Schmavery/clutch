...
let parseError =
    (prevCh, program: CharStream.t, cursor: option(int), ident, err) =>
  switch (cursor) {
  | Some(ch) when CharStream.ch(program) + 1 > ch && prevCh - 1 < ch =>
    Typing(ident)
...
let rec parse_args =
        (stream, acc, ident, cursor): parseResult(list(argT), errT) => {
...
let rec parse_program =
  ...
  | Some(c) =>
    parseError(
      ...
      cursor,
      "",
      Parse.append_char("Unexpected character ", c),
    )
...
and handleLabel = (program, args, prevCh, funcs, ident, cursor, cmds, labels) =>
  ...
  | [Var(name)] =>
    ...
    | (_cmd, line) =>
      parseError(
        ...
        ident,
        Printf.sprintf(
          "There is already a label with the name '%s' at line %i.",
          name,
          line,
        ),
      )
    | exception Not_found =>
      parse_program(
        ...
        StringMap.add(
          name,
          (List.length(cmds), CharStream.line(program)),
          labels,
        ),
      )
    }
  ...
  | _ =>
    parseError(
      ...
      cursor,
      ident,
      "label expects one variable as input",
    )
  };

let cmd =
    (state: stateT, cmd: cmdT, ~cb: (stateT, ~err: option(errT)) => unit) =>
  cmd.func(
    state,
    ~cb=
      fun
...
let rec run_until_error =
        (
          ...
          ~step=false,
          ~stop=?,
          ~cb: (stateT, ~err: option(errT)) => unit,
        ) =>
  switch (state.content[state.currCmd]) {
  | input =>
    cmd(state, input, ~cb=(state, ~err) =>
      switch (err) {
      | None =>
        switch (step, stop) {
        ...
        | (false, Some({contents: false})) =>
          ...
          Js.Global.setTimeout(
            () => run_until_error(state, ~step, ~stop?, ~cb),
            0,
          )
        }
